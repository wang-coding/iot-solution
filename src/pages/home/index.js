...
import './index.less';
import {Row, Input, Form, Card, Button, Modal, Table, Divider, Tag, Rate, Col, Checkbox} from "antd";
...
class Home extends React.Component {
    ...
    handleDetail = (link) => {
        window.location.href = `/#/equipment/detail/sensor/${link}`;
    }
    ...
    renderExhibition = () => {
        ...
        this.state.data.forEach((item,i)=>{
            let industry = item.map((item, index) => {
                item.key = index;
                return item
            });
            let table = <Card title={">> "+ industry[0].industry +" <<"}>
                <SolutionTable
                    dataSource={industry}
                    simple={false}
                    expandedRow="table"
        ...
        return (
            <div className="home">
                ...
                <Row className="exhibition">
                    <Card title=">> 查询结果 <<" hidden={this.state.search_res}>
                        <Table
